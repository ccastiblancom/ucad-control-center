...
import { ArrowLeft, Calendar, Search, RotateCcw, Target, AlertCircle, CheckCircle2, Home, Activity } from "lucide-react";
...
// Datos: Causas de Reingreso
const dataCausasReingreso = [
  { name: 'Manejo del Dolor', valor: 35, color: '#06b6d4' },
  { name: 'Infección Sitio Op.', valor: 30, color: '#0891b2' },
  { name: 'Intolerancia Vía Oral', valor: 20, color: '#0e7490' },
  { name: 'Fiebre sin Foco', valor: 15, color: '#155e75' },
];

// Datos: Tendencia Mensual de Reingresos
const dataTendenciaReingreso = [
  { mes: 'Ene', tasa: 1.8 },
  { mes: 'Feb', tasa: 2.2 },
  { mes: 'Mar', tasa: 1.5 },
  { mes: 'Abr', tasa: 1.2 },
...
// Registro de Pacientes Reingresados
const logReingresos = [
  { id: "REI-2026-008", paciente: "Julian Castro", cirugiaBase: "Apendicectomía", diasPostAlta: "4 días", motivo: "Dolor abdominal persistente", estado: "Auditado" },
  { id: "REI-2026-012", paciente: "Valentina Meza", cirugiaBase: "Herniorrafia", diasPostAlta: "12 días", motivo: "Seroma / Inflamación", estado: "Auditado" },
  { id: "REI-2026-020", paciente: "Mateo Ospina", cirugiaBase: "Pieloplastia", diasPostAlta: "7 días", motivo: "Vómito / Intolerancia", estado: "En Revisión" },
];

export default function Reingreso30DiasDashboard() {
  
  // Lógica de cumplimiento gerencial (Meta < 2%)
  const metaTasa = 2.0;
  const tasaActual = 1.0;
    ...
    <div className="relative min-h-screen w-full pb-12">
      ...
      <div className="relative z-10 max-w-7xl mx-auto mt-4 px-4">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h1 className="text-2xl font-bold text-[#324D6D] tracking-tight">% Reingreso a 30 Días</h1>
            <p className="text-sm text-slate-500">Unidad de Cirugía Pediátrica • Excelencia Clínica • Meta &lt; 2%</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between border-l-4 border-l-cyan-600">
            <div>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Egresos Auditados</p>
              <p className="text-3xl font-bold text-slate-800">285</p>
            </div>
            <Home className="w-8 h-8 text-cyan-600 opacity-20" />
          </div>
          <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between border-l-4 border-l-emerald-500">
            <div>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Tasa de Reingreso</p>
              <p className="text-3xl font-bold text-emerald-600">{tasaActual}%</p>
            </div>
            <RotateCcw className="w-8 h-8 text-emerald-500 opacity-20" />
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
            <h3 className="text-sm font-bold text-slate-700 mb-1">Causas de Reingreso</h3>
            <p className="text-[10px] text-slate-400 mb-6">Distribución de motivos para el reingreso hospitalario</p>
            <div className="h-64 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={dataCausasReingreso} layout="vertical" margin={{ left: -10, right: 30 }}>
                  ...
                  <Bar dataKey="valor" radius={[0, 4, 4, 0]} barSize={25}>
                    {dataCausasReingreso.map((entry, index) => (
          ...
          <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
            <h3 className="text-sm font-bold text-slate-700 mb-1">Evolución de la Tasa de Reingreso</h3>
            <p className="text-[10px] text-slate-400 mb-6">Tendencia mensual frente al límite institucional (2%)</p>
            <div className="h-64 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={dataTendenciaReingreso} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                  ...
                  <RechartsTooltip />
                  <Line type="monotone" dataKey={() => 2.0} stroke="#ef4444" strokeWidth={1} strokeDasharray="5 5" dot={false} name="Límite Máximo" />
                  <Line type="monotone" dataKey="tasa" stroke="#0891b2" strokeWidth={3} dot={{ r: 5, fill: "#0891b2", strokeWidth: 2, stroke: "#fff" }} name="Tasa Real" />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-50 bg-cyan-50/30 flex justify-between items-center">
            <h3 className="text-sm font-bold text-cyan-800 flex items-center gap-2">
              <AlertCircle className="w-4 h-4" /> Bitácora de Auditoría de Reingresos
            </h3>
            <button className="text-[10px] font-bold text-cyan-600 bg-white px-3 py-1.5 rounded-lg border border-cyan-200">Exportar Casos</button>
          </div>
            ...
            <table className="w-full text-xs text-left">
              <thead>
                <tr className="text-slate-500 border-b border-slate-100 uppercase tracking-wider font-semibold bg-slate-50/50">
                  ...
                  <th className="px-6 py-4">Cirugía de Origen</th>
                  <th className="px-6 py-4 text-center">Tiempo tras Alta</th>
                  <th className="px-6 py-4">Motivo Principal</th>
                  <th className="px-6 py-4 text-right">Estatus Auditoría</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {logReingresos.map((p, i) => (
                  <tr key={i} className="hover:bg-slate-50 transition-colors text-slate-600">
                    ...
                    <td className="px-6 py-4 text-slate-500">{p.cirugiaBase}</td>
                    <td className="px-6 py-4 text-center font-bold text-slate-700">{p.diasPostAlta}</td>
                    <td className="px-6 py-4 font-medium text-rose-600">{p.motivo}</td>
